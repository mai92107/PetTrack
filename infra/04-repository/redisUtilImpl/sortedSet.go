...
// 封裝方法：寫入一筆 資料（ZADD）
// 並設定過期時間為 24 小時
func (myCache *redisRepoImpl) ZAddData(ctx context.Context, key string, score float64, byteData []byte) error {
...
func (myCache *redisRepoImpl) KeyScan(ctx context.Context, pattern string) ([]string, error) {
	var cursor uint64
	var keys []string
...
func (myCache *redisRepoImpl) ZRangeByScore(ctx context.Context, key string, startTs, endTs int64) ([]string, error) {
...
func (myCache *redisRepoImpl) ZRemRangeByScore(ctx context.Context, key string, startTs, endTs int64) error {
	// 移除指定區間資料
