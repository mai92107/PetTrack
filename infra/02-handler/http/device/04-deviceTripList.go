...
func DeviceTrips(ctx request.RequestContext) {
	...
	datas, total, totalPages, err := handler.DeviceService.GetDeviceTrips(ctx.GetContext(), *userInfo, req.DeviceId, model.NewPageable(&req.Page, &req.Size, req.Direction, req.OrderBy))
	if err != nil {
